...
import wandb
# Disable all wandb logging calls
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    print(f"→ epoch() started in {mode} mode, {len(dataloader)} batches")

    for i_batch, datum in enumerate(dataloader):
        print(f"    DataLoader yielded batch {i_batch}")
        ...
        if i_batch % 10 == 0:
            print(f"    Processing batch {i_batch}/{len(dataloader)} | Moving data done")
        ...
        if i_batch % 10 == 0:
            print(f"    Processing batch {i_batch}/{len(dataloader)} | Augmentation done")
        ...
        if i_batch % 10 == 0:
            print(f"    Processing batch {i_batch}/{len(dataloader)} | Forward pass done")
        ...
        if i_batch % 10 == 0:
            print(f"    Batch {i_batch}/{len(dataloader)} done | Loss: {loss.item():.4f} | Time: {time.time() - batch_start:.2f}s")

    loss_avg /= num_exp
    acc_avg /= num_exp

    print(f"→ epoch() finished | Mode: {mode} | Avg Loss: {loss_avg:.4f} | Acc: {acc_avg*100:.2f}%")
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False):
    ...
    if args.optimizer == 'SGD':
        ...
        scheduler = LambdaLR(optimizer, lambda step: 0.5 * (1. + math.cos(math.pi * step / args.epoch_eval_train)) if step <= args.epoch_eval_train else 0, last_epoch=-1)

    
    criterion = nn.CrossEntropyLoss().to(args.device)

    print(f"→ DEBUG: Testing forward pass manually with images_train[0].unsqueeze(0)...")

    try:
        test_img = images_train[0].unsqueeze(0).to(args.device)  # Shape: (1, 3, 64, 64)

        print(f"→ DEBUG: test_img shape: {test_img.shape}, dtype: {test_img.dtype}, device: {test_img.device}")
        print(f"→ DEBUG: net type: {type(net)}")

        output = net(test_img)
        print(f"→ DEBUG: Forward pass successful, output shape: {output.shape}")
    ...
    print(f"→ Student {it_eval}: Starting {Epoch} epochs of training...")
    for ep in range(Epoch+1):
        ...
        if ep % 10 == 0 or ep == Epoch:
            print(f"    [Run {it_eval}] Epoch {ep}/{Epoch} | Loss: {loss_train:.4f} | Acc: {acc_train*100:.2f}% | Time: {time.time() - epoch_start:.2f}s")

        if ep == Epoch or (ep % 200 == 0 and ep > 0):
            with torch.no_grad():
                loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug=False)
                print(f"    [Run {it_eval}] Eval @ Epoch {ep}: Test Acc: {acc_test*100:.2f}%")
    ...
    print(f"✓ Finished Student {it_eval} in {total_train_time/60:.2f} min. Final Train Acc: {acc_train*100:.2f}% | Best Test Acc: {max_acc_test*100:.2f}%")
