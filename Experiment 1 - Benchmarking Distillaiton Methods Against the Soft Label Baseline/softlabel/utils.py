...
# get_dataset, get_network are used - slight changes to get_dataset to fix errors (download=False as when running on Iridis, the node is not connected to the internet)
# Information on training parameters taken from this code
...
def get_dataset(dataset, data_path, batch_size=1, args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        dst_train = datasets.CIFAR10('cifar10', train=True, download=False, transform=transform) # changed from download=True to download=False for COMP6258 Reproducability coursework
        dst_test = datasets.CIFAR10('cifar10', train=False, download=False, transform=transform)
        ...
        dst_train = datasets.CIFAR100('cifar100', train=True, download=False, transform=transform)  # no augmentation
        dst_test = datasets.CIFAR100('cifar100', train=False, download=False, transform=transform)
        class_names = dst_train.classes
        class_map = {x: x for x in range(num_classes)}
